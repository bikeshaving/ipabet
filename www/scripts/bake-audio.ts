...
async function synth(c: Clip): Promise<"baked" | "cached" | "fail"> {
	...
	// A non-empty file is baked; a zero-byte one is an interrupted write that
	// existsSync alone would treat as cached forever (silent-no-audio).
	if (existsSync(out) && statSync(out).size > 0) return "cached";
...
// Generate the manifest — only from clips whose audio actually exists on
// disk. A failed synth writes no file, and importing it would break bundling;
// exit nonzero so a failure can't ship a dangling import unnoticed.
const entries = [...clips.values()].filter((c) => existsSync(join(AUDIO_DIR, c.file)) && statSync(join(AUDIO_DIR, c.file)).size > 0);
if (entries.length !== clips.size) {
	console.error(`\n${clips.size - entries.length} clip(s) have no audio — not writing a manifest that imports them`);
	process.exit(1);
}
const lines: string[] = [
