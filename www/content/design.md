...
IPAbet covers every symbol on the standard IPA chart — consonants, vowels, diacritics, suprasegmentals, pulmonic and non-pulmonic — with one deliberate omission: <Glyph>ɧ</Glyph>, which has no stable articulation, so its variants are transcribed directly. extIPA is not yet covered. The full mapping is on [the chart](/chart), in [machine-readable form](/keys), and as [raw JSON](/ipabet.json).
