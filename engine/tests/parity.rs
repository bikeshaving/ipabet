...
/// The recorded vectors: what the JS suite actually typed.
#[test]
fn parity_vectors() {
...
/// glyphs, mark stacks) and keystrokes, generated by
/// js/scripts/fuzz-vectors.ts with the JS engine as oracle. The recorded
/// vectors only ever type IPA; these exist to catch the divergences that
/// live outside it — the emoji-backspace bug shipped precisely because
/// nothing typed an emoji.
#[test]
fn fuzz_vectors() {
...
fn fuzz_vectors_fresh() {
    match std::env::var("IPABET_FUZZ_VECTORS") {
        Ok(path) => replay_vector_file(&path),
        Err(_) => eprintln!("IPABET_FUZZ_VECTORS unset; fresh-seed fuzz skipped"),
    }
}
